...
const CLIPDROP_REIMAGINE_URL = 'https://clipdrop-api.co/reimagine/v1/reimagine';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!session && process.env.NODE_ENV === 'production') {
      ...
      if (!hasAuthCookie) {
        console.log('[AI Reimagine API] No auth cookie found in production');
      ...
      console.log('[AI Reimagine API] Auth cookie found, continuing despite no session');
    ...
    console.log('=== CLIPDROP AI REIMAGINE ===');
    ...
    try {
      ...
      // Check file size and provide better error message
      ...
      // Call Clipdrop Reimagine API - creates variations of the image
      const clipdropResponse = await fetch(CLIPDROP_REIMAGINE_URL, {
        method: 'POST',
        headers: {
      ...
      if (!clipdropResponse.ok) {
        ...
        if (errorText.includes('image exceeds the maximum height') || errorText.includes('image exceeds the maximum width')) {
          throw new Error('Image is too large for AI Reimagine. Please use an image smaller than 1024x1024 pixels.');
        } else if (errorText.includes('image_file')) {
          throw new Error('Invalid image format. Please use a standard image file (JPG, PNG).');
        } else {
          throw new Error(`AI Reimagine failed: ${errorText}`);
        }
      }

      // Get the reimagined image
      const reimaginedImageBuffer = await clipdropResponse.arrayBuffer();
      console.log('Reimagined image size:', reimaginedImageBuffer.byteLength, 'bytes');
      
      // Convert to base64 for easy handling
      const base64Image = Buffer.from(reimaginedImageBuffer).toString('base64');
      ...
      const processedFilename = `${nameWithoutExt}_reimagined.png`;
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to reimagine image'
      },
      { status: 500 }
    );
  }
