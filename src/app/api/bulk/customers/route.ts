...
/**
 * Bulk Customers API - V2 Optimized
 * Proxies to flora-im/v1/customers/bulk
 * Gets customers with stats and meta in optimized queries
 * 
 * @route GET /api/bulk/customers
 * @query role - Filter by role (default: customer)
 * @query search - Search by name, email, or username
 * @query per_page - Items per page (default: 100)
 * @query page - Page number (default: 1)
 * @query include_stats - Include order stats (default: true)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${FLORA_API_BASE}/flora-im/v1/customers/bulk`);
    ...
    console.error('Bulk customers API error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to fetch bulk customers' },
      { status: 500 }
    );
  }
}
