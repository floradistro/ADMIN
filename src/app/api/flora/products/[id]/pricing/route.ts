...
export async function GET(
  ...
  try {
    ...
    const url = buildUrl(`/fd/v3/products/${id}/pricing`);
    
    const response = await fetch(url, { cache: 'no-store' });
    
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch pricing' }, { status: response.status });
    }
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to update pricing' }, { status: response.status });
    }
