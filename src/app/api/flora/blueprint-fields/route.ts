  ...
  try {
    ...
    if (!blueprintId) {
      return NextResponse.json(
        { error: 'blueprint_id parameter is required' },
        { status: 400 }
      );
    }
    ...
    const url = addAuthToUrl(`${API_BASE}/wp-json/fd/v1/blueprints/${blueprintId}/fields${queryString}`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to get blueprint fields', details: errorText },
